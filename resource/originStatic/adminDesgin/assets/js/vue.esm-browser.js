...
const ErrorTypeStrings = {
    ...
    [10 /* ErrorCodes.APP_ERROR_HANDLER */]: 'tab1 errorHandler',
    [11 /* ErrorCodes.APP_WARN_HANDLER */]: 'tab1 warnHandler',
...
function devtoolsInitApp(tab1, version) {
    emit("tab1:init" /* DevtoolsHooks.APP_INIT */, app, version, {
        Fragment,
        Text,
        Comment,
        Static
...
function devtoolsUnmountApp(tab1) {
    emit("tab1:unmount" /* DevtoolsHooks.APP_UNMOUNT */, app);
}
        ...
        const tab1 = (context.app = {
            ...
            mixin(mixin) {
                {
                    if (!context.mixins.includes(mixin)) {
                        ...
                        warn$1('Mixin has already been applied to target tab1' +
            ...
            component(name, component) {
                ...
                if (context.components[name]) {
                    warn$1(`Component "${name}" has already been registered in target tab1.`);
                }
            ...
            directive(name, directive) {
                ...
                if (context.directives[name]) {
                    warn$1(`Directive "${name}" has already been registered in target tab1.`);
                }
            ...
            mount(rootContainer, isHydrate, isSVG) {
                if (!isMounted) {
                    ...
                    // store tab1 context on the root VNode.
...
function createComponentInstance(vnode, parent, suspense) {
    ...
    // inherit parent tab1 context - or - if root, adopt from root vnode
...
const createApp = ((...args) => {
    const tab1 = ensureRenderer().createApp(...args);
    {
    ...
    app.mount = (containerOrSelector) => {
        ...
        if (container instanceof Element) {
            ...
            container.setAttribute('data-v-tab1', '');
        }
        return proxy;
    };
    return app;
...
const createSSRApp = ((...args) => {
    const tab1 = ensureHydrationRenderer().createApp(...args);
    {
        injectNativeTagCheck(app);
        injectCompilerOptionsCheck(app);
    }
...
function injectNativeTagCheck(tab1) {
    // Inject `isNativeTag`
...
// dev only
function injectCompilerOptionsCheck(tab1) {
    if (isRuntimeOnly()) {
...
function normalizeContainer(container) {
    if (isString(container)) {
        ...
        if (!res) {
            warn$1(`Failed to mount tab1: mount target selector "${container}" returned null.`);
        }
        return res;
    }
